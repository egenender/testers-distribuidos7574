...
int AtendedorTesters::recibirRequerimiento() {
    ...
    int ret = msgrcv(this->cola_recepciones, &msg, sizeof(TMessageAtendedor) - sizeof(long), MTYPE_REQUERIMIENTO, 0);
    if(ret == -1) {
...
resultado_test_t AtendedorTesters::recibirResultado(int idTester) {
    ...
    int ret = msgrcv(this->cola_recepciones, &rsp, sizeof(resultado_test_t) - sizeof(long), idTester, 0);
    if(ret == -1) {
...
void AtendedorTesters::enviarAEspeciales(bool cuales[], int idDispositivo, int posicionDispositivo){
	...
	for (int i = 0; i < CANT_TESTERS_ESPECIALES; i++){
            ...
            ss.str("");
            int ret = msgsnd(this->cola_envios, &msg, sizeof(TMessageAssignTE) - sizeof(long), 0);
            if(ret == -1) {
...
TMessageAssignTE AtendedorTesters::recibirRequerimientoEspecial(int idEsp) {
    ...
    TMessageAssignTE msg;
    int ret = msgrcv(this->cola_recepciones, &msg, sizeof(TMessageAssignTE) - sizeof(long), idEsp, 0);
    if(ret == -1) {
        std::string error = std::string("Error al recibir requerimiento del atendedor. Error: ") + std::string(strerror(errno));
        Logger::error(error.c_str(), __FILE__);
        exit(0);
