...
int main(int argc, char *argv[]){
    ...
    Logger::notice("Envio id de mi dispositivo", name.str().c_str());
	...
	Logger::notice("Termine envio id de mi dispositivo, me pongo a leer", name.str().c_str());
    ...
    while (true) {
		...
		Logger::notice("Me pongo a leer del socket", name.str().c_str());   
		char* buffer_act = (char*) buffer;
		while( (leido = read(fd, buffer_act, size - acumulado)) > 0){
			acumulado += leido;
			buffer_act = buffer_act + leido;
		}
		Logger::notice("Termine de leer mensaje. Envio a la cola correspondiente", name.str().c_str());   
		...
		Logger::notice("Termine envio a la cola", name.str().c_str());   
	}
