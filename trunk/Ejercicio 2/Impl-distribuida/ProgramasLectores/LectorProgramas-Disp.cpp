...
int main(int argc, char** argv) {
	int idDispositivo = atoi(argv[1]);
	Logger::initialize(logFileName.c_str(), Logger::LOG_DEBUG);
	std::stringstream nombre;
	nombre << __FILE__ << " " << idDispositivo ;
	Logger::notice("Inicia el procesamiento, cargando IPCS" , nombre.str().c_str());
	
    key_t key = ftok(ipcFileName.c_str(), MSGQUEUE_PROGRAMAS_SOCKET);
   ...
   Logger::notice("Procesamiento inicial completo, ejecutando ciclo principal" , nombre.str().c_str());
    ...
    while (true){
        ...
        int ok_read = msgrcv(cola_lectura, &msg, sizeof(TMessageAtendedor) - sizeof(long), idDispositivo, 0);
        if (ok_read == -1){
			exit(0);
		}
		std::stringstream ss;
		ss << "Se recibe nuevo programa para dispositivo " << msg.idDispositivo;
