...
int main (int argc, char* argv[]) {
	...
	while(true) {
        ...
        ss << "Los testers especiales que se requieren ya estan registrados";
        Logger::debug(ss.str(), __FILE__); ss.str(""); ss.clear();

        ss << "Ahora me fijo si los testers especiales que se requieren estan disponibles";
        Logger::debug(ss.str(), __FILE__); ss.str(""); ss.clear();
        ...
        do {
            ...
            semTabla.p();
            for (int i = 0; i < msg.cantTestersEspecialesAsignados; i++) {
                if (!tablaTestersEspReg->disponibles[msg.idTestersEspeciales[i] - ID_TESTER_ESP_START + MAX_TESTER_COMUNES]) {
                    semTabla.v();
                    ss << "Espero a que este disponible el tester especial id " << msg.idTestersEspeciales[i];
                    Logger::notice(ss.str(), __FILE__); ss.str(""); ss.clear();
                    // Vuelvo a poner disponible los que tome
                    while(--i >= 0) tablaTestersEspReg->disponibles[msg.idTestersEspeciales[i] - ID_TESTER_ESP_START + MAX_TESTER_COMUNES] = true;
                    exito = false;
                    break;
                }
                tablaTestersEspReg->disponibles[msg.idTestersEspeciales[i] - ID_TESTER_ESP_START + MAX_TESTER_COMUNES] = false;
            }
            semTabla.v();
            usleep(1000 * 1000);
        } while (!exito);
        ...
        for (int i = 0; i < msg.cantTestersEspecialesAsignados; i++) {
            ...
            ss.str(""); ss.clear();
            ss << "Se enviará requerimiento especial al tester especial " << msg.mtype << " para que testee al dispositivo " << msg.idDispositivo;
            Logger::notice(ss.str(), __FILE__);
