...
int main(int argc, char** argv) {    
    ...
    AtendedorTesters atendedor( id, config );
    ...
    PlanillaAsignacionTesterComun planillaAsignacion( id, config );
    ...
    while(true) {
        ...
        if (resul.value == Constantes::RESULTADO_GRAVE){
        ...
        } else if (resul.value == Constantes::SEGUIR_TESTEANDO) {
            int cant_testers = 0;
            ...
            for (int i = 0; i < cantTestersEspeciales; i++){
                    if(los_testers[i])
                            ss << i + Constantes::ID_TESTER_ESP_START << " ";
            }
