...
int main(int argc, char** argv) {
    ...
    key_t key = ftok(ipcFileName.c_str(), MSGQUEUE_ESCRITURA_RESULTADOS_SOCKET);
   ...
   Logger::notice("Procesamiento inicial completo, ejecutando ciclo principal" , nombre.str().c_str());
    ...
    while (true){
        ...
        int ok_read = msgrcv(cola_lectura, &msg, sizeof(TMessageAtendedor) - sizeof(long), idTester, 0);
        if (ok_read == -1){
			exit(0);
		}
		std::stringstream ss;
		ss << "Se recibe resultados desde el dispositivo " << msg.idDispositivo;
