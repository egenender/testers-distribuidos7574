...
int main(int argc, char** argv) {
    ...
    key_t key = ftok(ipcFileName.c_str(), MSGQUEUE_ESCRITURA_RESULTADOS_ENVIO);
    int cola_lectura = msgget(key, 0660);
    key = ftok(ipcFileName.c_str(), MSGQUEUE_ESCRITURA_RESULTADOS_SOCKET);
   ...
   Logger::notice("Procesamiento inicial completo, ejecutando ciclo principal" , nombre.str().c_str());
    ...
    while (true){
		...
		std::stringstream ss;
		ss << "Se envia resultados desde el dispositivo " << msg.idDispositivo;
